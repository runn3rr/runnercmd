...
pip install -r "%~dp0\src\requirements.txt" >nul 2>&1
cls
...
if /i "%command%"=="credits" ( call "%~dp0\credits.bat" called && goto :input )
if /i "%command%"=="spamping" ( call "%~dp0\spamping.bat" called && goto :input )
if /i "%command%"=="mods" ( call "%~dp0\mods.bat" called && goto :input)
